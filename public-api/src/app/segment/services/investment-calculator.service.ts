...
import { OptimalInvestmentEquation, Segment } from '../../../models/api-interfaces';
...
export class InvestmentCalculatorService {
  ...
  calculateInvestments(segments: Segment[], equation: OptimalInvestmentEquation): Observable<Segment[]> {
    return of(segments).pipe(
      ...
      map(segments => segments.map(segment => ({
        ...segment,
        optimalInvestment: this.getOptimalInvestment(segment, segments, equation),
        expectedLossBeforeInvestment: this.getExpectedLossBeforeInvestment(segment)
      }))),
      map(segments => segments.map(segment => ({
        ...segment,
        expectedLossWithInvestment: this.getLossWithInvestment(segment, segments, equation)
      }))),
      map(segments => segments.map(segment => ({
        ...segment,
  ...
  calculateInvestmentsWithoutSegmentation(segments: Segment[], equation: OptimalInvestmentEquation): Observable<Partial<Segment>> {
    return of({
      ...
      map(segmentation => ({
        ...segmentation,
        expectedLossBeforeInvestment: this.getExpectedLossBeforeInvestment(segmentation),
        optimalInvestment: this.getOptimalInvestment(segmentation, segments, equation)
      })),
      map(segmentation => ({
        ...segmentation,
        expectedLossWithInvestment: this.getLossWithInvestment(segmentation, segments, equation),
      })),
      map(segmentation => ({
        ...segmentation,
  ...
  private getExpectedLossBeforeInvestment(segment: Segment | Partial<Segment>): number {
  ...
  private getOptimalInvestment(segment: Segment | Partial<Segment>, segments: Segment[], investmentEquation: OptimalInvestmentEquation): number {
    const diff = nerdamer(`diff(${investmentEquation.breachProbabilityFunction}, z)`);
    const optimalInvestmentEquation = nerdamer(investmentEquation.optimalInvestmentEquation, { S: `(${diff.toString()})` }).evaluate();
  ...
  private getLossWithInvestment(segment: Segment | Partial<Segment>, segments: Segment[], investmentEquation: OptimalInvestmentEquation): number {
    // Si*Li
    const lossWithInvestment = nerdamer(
      ...
      Li: segment.value,
      v: segment.calculatedVulnerability,
      z: segment.optimalInvestment / (segment.value / this.getTotalValue(segments))
    }, 'numer');
  ...
  private getWeightedVulnerability(segments: Segment[]): number {
    if (segments.every(segment => Number.isFinite(segment.calculatedVulnerability))) {
      const weightedAverage = segments.reduce((pre, curr) => pre + curr.value * curr.calculatedVulnerability, 0);
      return Math.round((weightedAverage / this.getTotalValue(segments)) * 100) / 100;
    } else {
      return 0;
    }
  }
