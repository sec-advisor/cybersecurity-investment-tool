...
export class SegmentDefinitionService {
  constructor(
    @InjectModel('segment-definitions')
    private readonly segmentDefinitionModel: Model<SegmentDefinition>,
  ) {}
        ...
        segments.map(
          (segment) =>
            ({
              ...
              valueEstimation:
                segment.valueEstimation?.inputs?.length > 0
                  ? segment.valueEstimation
                  : undefined,
  ...
  calculateValue(
    ...
    try {
      const splittedCalculationString =
        segment.valueEstimation.calculation.split(' ');
      if (
        splittedCalculationString &&
        keyValuePairs &&
        keyValuePairs.every((pair) => Number.isFinite(pair.value))
      ) {
        ...
        const mergedCalculation = splittedCalculationString.reduce(
          (pre, curr) => pre + ` ${curr}`,
          '',
        );
