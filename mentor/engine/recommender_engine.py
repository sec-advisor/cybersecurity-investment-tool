import numpy as np
from engine.helpers.similarity_functions import (
    euclidean_distance,
    jaccard_similarity,
    cosine_similarity,
    manhattan_distance,
...
class RecEngine:
    ...
    def calc_customer_index(self):
        ...
        deployment_time_index = sh.calculate_index(
            sh.deployment_dict, cs.deploymentTime
        )
    ...
    def calc_service_index(self):
        ...
        for s in sh.services:
            ...
            s.total_index = [
                ...
                leasing_period_index,
                self.customer.budget - s.price,
            ]
    ...
    def calc_similarity(self):
        ...
        customer_profile_weights = [
            cs.deploymentTimeWeight,
            cs.leasingPeriodWeight,
            cs.budgetWeight,
        ]
        ...
        for s in sh.services:
            s.weighted_total_index = np.multiply(
                s.total_index, customer_profile_weights
            )

            # Calculation of Similarity
            s.euclideanDistance = euclidean_distance(
                cs.weighted_total_index, s.weighted_total_index
            )
            s.jaccardSimilarity = jaccard_similarity(
                cs.weighted_total_index, s.weighted_total_index
            )
            s.cosineSimilarity = cosine_similarity(
                cs.weighted_total_index, s.weighted_total_index
            )
            s.manhattanDistance = manhattan_distance(
                cs.weighted_total_index, s.weighted_total_index
            )
            s.pearsonCorrelation = pearson_correlation(
                cs.weighted_total_index, s.weighted_total_index
            )
            s.minkowskiDistance = minkowski_distance(
                cs.weighted_total_index,
                s.weighted_total_index,
                len(s.weighted_total_index),
            )

            # Rating is given based on a normalization of all ratings
            s.rating = np.linalg.norm(
                [
                    s.euclideanDistance,
                    s.jaccardSimilarity,
                    s.cosineSimilarity,
        ...
        sorted_normalized_similarities = [
            (k, similarity[k]) for k in sorted(similarity, key=similarity.get)
        ]
                ...
                if s.id == k:
                    result.append(
                        s.serialize(
                            ...
                            s.pearsonCorrelation,
                            s.minkowskiDistance,
                            json.dumps(s.weighted_total_index.tolist()),
                        )
                    )
        return {
            "recommendedServices": result
            if topNServices <= 0
            else result[0:topNServices],
