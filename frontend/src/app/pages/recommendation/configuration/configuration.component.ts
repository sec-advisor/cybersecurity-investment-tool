...
export class ConfigurationComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    this.stream$ = this.storageService.getBusinessProfile().pipe(
      switchMap((profile) =>
        this.segmentSource$.pipe(
          switchMap((segment) =>
            this.segmentDefinitionDataService.getSegmentDefinitions().pipe(
  ...
  private createForm(
    ...
    return this.formGroup.group({
      ...
      serviceType: [
        segment.recommendationProfile?.serviceType?.[0],
        [Validators.required],
      ],
      attackType: [
        segment.recommendationProfile?.attackType?.[0] || attacks?.[0],
        [Validators.required],
      ],
      deploymentTime: [
        segment.recommendationProfile?.deploymentTime,
        [Validators.required],
      ],
      leasingPeriod: [
        segment.recommendationProfile?.leasingPeriod,
        [Validators.required],
      ],
      investment: [
        segment.recommendationProfile?.budget || segment.optimalInvestment,
        [Validators.required],
      ],
    });
  }
