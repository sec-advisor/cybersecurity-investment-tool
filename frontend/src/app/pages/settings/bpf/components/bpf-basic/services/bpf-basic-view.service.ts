...
export class BpfBasicViewService {
  ...
  private readonly updateValueSubject$ = new BehaviorSubject<string | undefined>(undefined);
  ...
  getViewModel(): Observable<BpfBasicViewModel> {
    return this.updateValueSubject$.pipe(
      ...
      map(({ value, variables }) => ({ value, variables, form: this.createForm(variables) })),
      switchMap(({ value, variables, form }) => merge(
        of({ variables, form }),
  ...
  private createForm(variables: BpfBasicVariable[]): FormGroup {
    return this.formBuilder.group(variables.reduce((pre, variable) => ({
      ...pre,
  ...
  private handleValueChanges(formValue: [formName: number], value: string): void {
    const splittedValue = this.getSplittedValue(value);
    const updatedValue = Object.entries(formValue).map(([key, value]) => ({ key: key as BpfBasicFormName, value }))
    const newBpfValue = splittedValue.map(s => this.replaceValueParts(s, updatedValue))
      .reduce((pre, curr) => `${pre}${curr}`, '')

    this.valueSubject$.next(newBpfValue);
  }

  private replaceValueParts(valuePart: string, formValue: { key: BpfBasicFormName, value: number }[]): string {
  ...
  private getVariables(value: string): BpfBasicVariable[] {
    const variables = [
      { name: 'v', description: 'This is variable v', controlName: BpfBasicFormName.V },
      { name: 'z', description: 'This is variable z', controlName: BpfBasicFormName.Z },
      { name: 'L', description: 'This is variable L', controlName: BpfBasicFormName.L },
    ];
  ...
  private getSplittedValue(value: string): string[] {
    const splitters = ['+', '-', '/', '*', '(', ')'];
    splitters.forEach(splitter => value = value.replaceAll(splitter, '~' + splitter + '~'));
    return value.split('~').filter(part => !!part);
  }

  private getVariableValue(name: string, splittedValue: string[]): number {
    const value = splittedValue.find(s => s.includes(name))?.replace(name, '');
